...
export default function WordStudyTutorial({ onClose }: WordStudyTutorialProps) {
  ...
  const getNextExpectedSplit = () => {
    const remainingSplits = currentItem.expectedSplits.filter(
      (s) => !splitPoints.includes(s),
    );
  ...
  const handleSplitClick = (index: number) => {
    ...
    if (newSplits.length === maxSplits) {
      ...
      setInstructionStep(
        Math.min(instructionStep + 1, currentItem.instructions.length - 1),
      );
    } else {
      setInstructionStep(
        Math.min(instructionStep + 1, currentItem.instructions.length - 1),
        ...
        <div className="bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-lg sm:rounded-xl p-3 sm:p-4 mb-4 sm:mb-6 shadow-lg animate-slide-down relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-1 bg-white/30">
            <div
              ...
              style={{
                width: `${((instructionStep + 1) / currentItem.instructions.length) * 100}%`,
              }}
            />
          </div>
