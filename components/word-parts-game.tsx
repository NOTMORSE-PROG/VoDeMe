...
function CharacterHand({
  ...
  return (
    <div className="fixed bottom-2 left-2 sm:bottom-4 sm:left-4 z-50 flex items-end gap-1 sm:gap-2">
      {/* Hand holding pencil */}
      <div
        className={`relative transition-all duration-300 ${isWriting ? "animate-pencil-write" : ""}`}
      >
      ...
      <style jsx>{`
        ...
        .animate-pencil-write {
          animation: pencil-write 0.3s ease-in-out infinite;
        }
        @keyframes pop-in {
          0% {
            transform: scale(0);
...
const Notebook = React.forwardRef<
  HTMLDivElement,
  {
    label: string;
    color: string;
      ...
      >
        ...
        <div className="absolute -left-1 sm:-left-2 top-2 sm:top-4 bottom-2 sm:bottom-4 w-2 sm:w-4 flex flex-col justify-center gap-1 sm:gap-2">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="w-2 h-2 sm:w-3 sm:h-3 bg-gray-400 rounded-full border border-gray-500 sm:border-2"
            />
          ))}
        </div>
        ...
        {/* Label tab */}
        <div
          className={`absolute -top-2 sm:-top-3 left-1/2 -translate-x-1/2 px-2 sm:px-3 py-0.5 sm:py-1 rounded-t-lg ${colors.bg} border border-b-0 sm:border-2 ${colors.border}`}
        >
          <span
            className={`text-[10px] xs:text-xs sm:text-sm font-bold ${colors.text}`}
          >
            {label}
          </span>
        </div>
        ...
        <style jsx>{`
          ...
          .animate-write-in {
            animation: write-in 0.3s ease-out;
          }
          @keyframes bounce-in {
            0% {
              transform: scale(0);
...
export default function WordPartsGame({ onBack }: WordPartsGameProps) {
  const [gameState, setGameState] = useState<
    "start" | "levelSelect" | "playing" | "levelComplete" | "gameComplete"
  >("start");
  ...
  const [handEmotion, setHandEmotion] = useState<"neutral" | "happy" | "sad">(
    "neutral",
  );
  const [showTutorial, setShowTutorial] = useState(false);
  const [levelStatuses, setLevelStatuses] = useState<
    Record<number, "locked" | "unlocked" | "completed">
  >({
    1: "unlocked",
    2: "locked",
    3: "locked",
  ...
  const [userSegments, setUserSegments] = useState<{
    ...
    base: string;
    suffix: string;
  } | null>(null); // Segments created by user
  ...
  const [stickyNotePlacement, setStickyNotePlacement] = useState<
  ...
  const handleSegmentPointerDown = (
    ...
    segmentType: "prefix" | "base" | "suffix",
  ) => {
  ...
  const handleSegmentPointerUp = (e: React.PointerEvent) => {
    ...
    if (targetNotebook) {
      const segmentValue =
        draggingSegment === "prefix"
          ? userSegments.prefix
          : draggingSegment === "base"
            ? userSegments.base
            : userSegments.suffix;
      setPlacements((prev) => ({
        ...prev,
        [targetNotebook as string]: segmentValue,
      }));
    }
  ...
  const handleLevel1Submit = () => {
    ...
    const correctPrefix =
      item.prefix === "none" ? null : item.prefix.replace(/-/g, "");
  ...
  const handleNext = async () => {
    if (currentItem < 9) {
      ...
      try {
        const result = await saveLevelProgress(
          "wordstudyjournal",
          currentLevel,
          score,
          10,
  ...
  const handleNextLevel = async () => {
    ...
    if (levelStatus === "locked") {
      toast.error(
        `Level ${nextLevel} is locked! Complete Level ${currentLevel} to unlock it.`,
      );
      return;
    }
    ...
    return (
      <ClassroomBackground>
        <div className="min-h-screen flex flex-col items-center justify-center p-4 pt-56">
          ...
          {/* Title on Chalkboard */}
          <div className="absolute top-12 xs:top-14 sm:top-16 md:top-18 left-1/2 -translate-x-1/2 text-center z-20 px-4 w-full">
            <h1
          ...
          <div className="bg-white/95 backdrop-blur rounded-2xl sm:rounded-3xl p-4 xs:p-5 sm:p-6 md:p-8 max-w-2xl w-full mx-4 shadow-2xl border-2 sm:border-4 border-amber-400 animate-fade-in-up mt-4 sm:mt-6 md:mt-8">
            ...
            <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
              <div className="flex items-start gap-2 sm:gap-3 bg-yellow-50 p-3 sm:p-4 rounded-xl border border-yellow-200 sm:border-2">
                ...
                <div>
                  <p className="font-bold text-yellow-700 text-sm sm:text-base">
                    Level 1: Deconstruction
                  </p>
                  <p className="text-gray-600 text-xs sm:text-sm">
                    <strong>Drag</strong> word parts into the correct notebooks
                    (Prefix, Base, Suffix). Place all parts, then click Submit!
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2 sm:gap-3 bg-pink-50 p-3 sm:p-4 rounded-xl border border-pink-200 sm:border-2">
                ...
                <div>
                  <p className="font-bold text-pink-700 text-sm sm:text-base">
                    Level 2: Classification
                  </p>
                  <p className="text-gray-600 text-xs sm:text-sm">
                    <strong>Drag</strong> sticky notes to Derived Form or
                    Inflected Form notebook, then Submit!
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2 sm:gap-3 bg-blue-50 p-3 sm:p-4 rounded-xl border border-blue-200 sm:border-2">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-xs sm:text-sm">
                    <strong>Drag</strong> the correct word into the sentence
                    blank, then Submit!
                  </p>
                </div>
              </div>
            ...
            <div className="bg-amber-50 p-3 sm:p-4 rounded-xl border border-amber-300 sm:border-2 mb-4 sm:mb-6">
              <p className="text-center text-amber-700 text-xs sm:text-sm">
                <strong>💡 Tip:</strong> You can drag items anywhere! After
                placing all items, click Submit to see if you're correct. Green
                = correct ✅, Red = wrong ❌
              </p>
            </div>
    ...
    return (
      <ClassroomBackground>
        <div className="min-h-screen flex flex-col items-center justify-center p-4 pt-56">
          ...
          {/* Title on Chalkboard */}
          <div className="absolute top-12 xs:top-14 sm:top-16 md:top-18 left-1/2 -translate-x-1/2 text-center z-20 px-4">
            <h1
              ...
              return (
                <div key={config.level} className="relative">
                  ...
                  >
                    {/* Lock Icon */}
                    {isLocked && (
                      <div className="absolute top-3 right-3 sm:top-4 sm:right-4 text-2xl sm:text-3xl md:text-4xl">
                        🔒
                      </div>
                    )}
                    ...
                    {/* Title */}
                    <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-amber-700 mb-1 sm:mb-2">
                      {config.title}
                    </h3>
                    ...
                    {/* Difficulty Badge */}
                    <div
                      className={`inline-block px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-white font-bold text-xs sm:text-sm bg-gradient-to-r ${config.color}`}
                    >
          ...
          <div className="mt-4 bg-amber-50/90 backdrop-blur border-2 border-amber-400 rounded-xl p-4 max-w-2xl text-center space-y-2">
            ...
            <p className="text-amber-600 text-xs sm:text-sm">
              ⓘ Only your first attempt at each level will be recorded for
              scoring
            </p>
          </div>
          ...
          <div className="bg-white/95 backdrop-blur rounded-3xl p-8 max-w-md shadow-2xl border-4 border-green-400 text-center animate-pop-in">
            ...
            <div className="bg-gradient-to-r from-blue-100 to-purple-50 rounded-2xl p-6 my-6">
              ...
              <p className="text-sm font-semibold mt-2 text-blue-700">
                {currentLevel < 3
                  ? `🎊 Level ${nextLevelNum} Unlocked!`
                  : "🏆 All Levels Complete!"}
              </p>
              {attemptInfo && !attemptInfo.isFirstAttempt && (
                <p className="text-xs sm:text-sm text-gray-600 mt-2">
                  ⓘ Only first attempt is recorded (
                  {(attemptInfo.recordedScore ?? 0) * 10} pts)
                </p>
              )}
              {attemptInfo && attemptInfo.isFirstAttempt && (
            ...
            <p className="text-lg text-gray-600 mb-6">
              {currentLevel < 3
                ? `Ready for Level ${nextLevelNum}: ${levelNames[nextLevelNum]}?`
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-6 text-left space-y-2">
              <p className="flex justify-between text-gray-700">
                ...
                <span className="font-bold text-yellow-600">
                  {levelScores[1] * 10} pts
                </span>
              </p>
              <p className="flex justify-between text-gray-700">
                <span>Level 2 (Classification):</span>
                <span className="font-bold text-pink-600">
                  {levelScores[2] * 10} pts
                </span>
              </p>
              <p className="flex justify-between text-gray-700">
      ...
      if (splitPoints.includes(index)) {
        ...
        if (splitPoints.length < maxSplits) {
          ...
          if (newSplits.length === maxSplits) {
            ...
            const correctPrefix =
              item.prefix === "none" ? "" : item.prefix.replace(/-/g, "");
            const correctBase = item.base;
            const correctSuffix = item.suffix.replace(/-/g, "");
      ...
      <>
        <ClassroomBackground>
          <div className="min-h-screen flex flex-col p-4 pt-56">
            ...
            {/* Chalkboard Content */}
            <div className="absolute top-14 xs:top-16 sm:top-18 md:top-20 left-1/2 -translate-x-1/2 text-center z-20 w-full max-w-2xl px-4">
              <p className="text-white text-xs xs:text-sm sm:text-base mb-1 sm:mb-2 opacity-80">
                {!userSegments
                  ? "Click between letters to split the word"
                  : "Drag each part to the correct notebook"}
              </p>
            ...
            <div className="flex-1 flex flex-col items-center justify-center mt-8">
              ...
              {!userSegments && !submitted && (
                <div className="bg-white/95 backdrop-blur rounded-2xl p-6 sm:p-8 shadow-xl border-4 border-amber-300 mb-8 animate-fade-in-up max-w-3xl w-full">
                  <div className="flex items-center justify-between mb-6">
                    <p className="text-center text-gray-600 text-sm flex-1">
                      <span className="font-bold text-amber-700">
                        ✏️ Click between letters
                      </span>{" "}
                      to split the word into parts!
                      {item.prefix !== "none"
                        ? " (Need 2 splits for Prefix + Base + Suffix)"
                        : " (Need 1 split for Base + Suffix)"}
                    </p>
                  ...
                  <p className="text-center text-gray-500 text-xs mt-4">
                    Splits marked: {splitPoints.length} /{" "}
                    {item.prefix !== "none" ? "2" : "1"}
                  </p>
                </div>
              )}
              ...
              {userSegments && !submitted && (
                <div
                  className={`bg-white/95 backdrop-blur rounded-2xl p-4 sm:p-6 shadow-xl border-4 mb-8 ${
                    splitError
                ...
                >
                  ...
                  <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                    {item.prefix !== "none" && userSegments.prefix && (
                      <div
                        ...
                        onPointerDown={(e) =>
                          handleSegmentPointerDown(e, "prefix")
                        }
                    ...
                    <div
                      draggable={
                        !Object.values(placements).includes(userSegments.base)
                      }
                    ...
                    <div
                      draggable={
                        !Object.values(placements).includes(userSegments.suffix)
                      }
                      onDragStart={() => setDraggingSegment("suffix")}
                      onDragEnd={() => setDraggingSegment(null)}
                      onPointerDown={(e) =>
                        handleSegmentPointerDown(e, "suffix")
                      }
                      onPointerMove={handleSegmentPointerMove}
                      onPointerUp={handleSegmentPointerUp}
                      className={`draggable-no-callout px-2 py-1.5 xs:px-3 xs:py-2 sm:px-4 sm:py-3 rounded-lg border-2 font-bold text-sm xs:text-base sm:text-lg md:text-xl transition-all duration-200 shadow-md bg-amber-50 border-amber-300 text-gray-800 touch-none ${
                  ...
                  {splitError && (
                    <div className="mt-4 bg-red-50 border-2 border-red-300 rounded-lg p-3 text-center animate-fade-in-up">
                      <p className="text-red-700 text-xs mb-2 font-semibold">
                        💡 Hint: Correct split should be:
                      </p>
              ...
              <div className="text-center">
                ...
                {submitted && (
                  <div className="animate-fade-in-up">
                    <p
                      className={`text-2xl font-bold mb-4 ${getValidationState("base") === "correct" && getValidationState("suffix") === "correct" && getValidationState("prefix") === "correct" ? "text-green-500" : "text-red-500"}`}
                    >
                      {getValidationState("base") === "correct" &&
                      getValidationState("suffix") === "correct" &&
                      getValidationState("prefix") === "correct"
                        ? "✓ Perfect! All parts correct!"
                        : "✗ Check the red notebooks - those are wrong!"}
                    </p>
                    <button
                      onClick={handleNext}
      ...
      <>
        <ClassroomBackground>
          <div className="min-h-screen flex flex-col p-4 pt-56">
            ...
            {/* Chalkboard Content */}
            <div className="absolute top-16 xs:top-18 sm:top-20 md:top-22 left-1/2 -translate-x-1/2 text-center z-20 w-full max-w-2xl px-2 sm:px-4">
            ...
            <div className="flex-1 flex flex-col items-center justify-center mt-8">
                ...
                <div className="mb-8 animate-pop-in">
                  <div
                    ...
                    className={`draggable-no-callout relative w-20 h-20 xs:w-24 xs:h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 p-2 sm:p-3 rounded-sm shadow-lg transition-all duration-300 touch-none ${
                      noteColors[currentItem % 4] === "yellow"
                        ? "bg-yellow-200 shadow-yellow-300"
                        : noteColors[currentItem % 4] === "pink"
                          ? "bg-pink-200 shadow-pink-300"
                          : noteColors[currentItem % 4] === "blue"
                            ? "bg-blue-200 shadow-blue-300"
                            : "bg-green-200 shadow-green-300"
                    } ${draggingStickyNote ? "opacity-50 scale-90" : ""} ${
                      stickyNotePlacement !== null
                        ? "opacity-0 pointer-events-none"
                        : "cursor-grab active:cursor-grabbing hover:scale-105 hover:-rotate-3"
                    }`}
                    style={{
                      transform:
                        stickyNotePlacement !== null
              ...
              <div className="flex flex-wrap justify-center gap-6 sm:gap-8 mb-8">
                <Notebook
                  ref={(el) => {
                    dropZoneRefs.current["derived"] = el;
                  }}
                  label="Derived Form"
                  color="purple"
                  placedContent={
                    stickyNotePlacement === "derived" ? item.word : null
                  }
                  validationState={
                    submitted ? (item.isDerived ? "correct" : "wrong") : null
                  }
                  showValidation={
                    submitted && stickyNotePlacement === "derived"
                  }
                ...
                <Notebook
                  ref={(el) => {
                    dropZoneRefs.current["inflected"] = el;
                  }}
                  label="Inflected Form"
                  color="pink"
                  placedContent={
                    stickyNotePlacement === "inflected" ? item.word : null
                  }
                  validationState={
                    submitted ? (!item.isDerived ? "correct" : "wrong") : null
                  }
                  showValidation={
                    submitted && stickyNotePlacement === "inflected"
                  }
                  isDragging={draggingStickyNote}
                  onDrop={(e) => {
                    e.preventDefault();
              ...
              <div className="bg-white/80 backdrop-blur rounded-xl p-3 sm:p-4 max-w-2xl text-center text-xs sm:text-sm text-gray-600 mb-4 sm:mb-6 mx-4">
                <p className="whitespace-nowrap overflow-x-auto">
                  <span className="font-bold text-purple-600">Derived:</span>{" "}
                  Creates new word class or meaning (e.g., teach → teacher)
                </p>
                <p className="whitespace-nowrap overflow-x-auto">
                  <span className="font-bold text-pink-600">Inflected:</span>{" "}
                  Shows tense, number, comparison (e.g., walk → walked)
                </p>
              </div>
      ...
      <>
        <ClassroomBackground>
          <div className="min-h-screen flex flex-col p-4 pt-56">
            ...
            {/* Chalkboard Content */}
            <div className="absolute top-14 xs:top-16 sm:top-18 md:top-20 left-1/2 -translate-x-1/2 text-center z-20 w-full max-w-3xl px-4">
            ...
            <div className="flex-1 flex flex-col items-center justify-center mt-8">
              ...
              {!submitted && (
                <div className="mb-6">
                  <p className="text-center text-gray-600 text-sm mb-3">
                    Drag a word to the blank above:
                  </p>
                      ...
                      <div
                        ...
                        className={`draggable-no-callout px-3 py-2 xs:px-4 xs:py-2.5 sm:py-3 rounded-xl font-semibold text-sm xs:text-base transition-all shadow-md border-2 border-amber-200 touch-none ${
                          ...
                          sentenceBlank === word
                            ...
                            : "bg-white hover:bg-amber-50 text-gray-700 cursor-grab active:cursor-grabbing hover:scale-105"
                        }`}
                      >
                        {word}
                      </div>
              ...
              <div className="text-center">
                ...
                {submitted && (
                  <div className="animate-fade-in-up">
                    <p
                      className={`text-2xl font-bold mb-2 ${sentenceBlank === item.correctAnswer ? "text-green-500" : "text-red-500"}`}
                    >
                      {sentenceBlank === item.correctAnswer
                        ? "✓ Perfect!"
                        : "✗ Not quite!"}
